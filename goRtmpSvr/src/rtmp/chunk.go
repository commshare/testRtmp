...
func readChunkHeader(br *bufio.Reader) (*ChunkHeader, error) {
	...
	if mh.MessageTypeID == 18 {
		fmt.Println("got message type id 18")
	}
	ch := &ChunkHeader{
		BasicHeader:   bh,
		MessageHeader: mh,
...
func readBasicHeader(br *bufio.Reader) (*BasicHeader, error) {
	fmt.Println(" readBasicHeader begin: try to read one byte ")
	...
	h := new(BasicHeader)
	h.FMT = uint8(x) >> 6
	fmt.Printf("readBasicHeader fmt %d\n",h.FMT)
	var basic_header_byte = fmt.Sprintf("readBasicHeader [%x] (% 08b) fmt %d\n",x,x,h.FMT)
	fmt.Println(basic_header_byte)
	...
	fmt.Printf("readBasicHeader fmt %d csid %d ChunkStreamID %d \n",h.FMT,csid,h.ChunkStreamID);
...
func readMessageHeader(br *bufio.Reader, ifmt uint8) (*MessageHeader, error) {
	mh := new(MessageHeader)
	fmt.Printf("readMessageHeader fmt %d \n",ifmt)
	switch ifmt {
	case 0:
		x := make([]byte, 11)
		...
		var xxxx =fmt.Sprintf("fmt 0 ,message header 11 bytes:[%x]", x)
		fmt.Println(xxxx)
