...
const (
	MessageSetChunkSize              MessageType = 1 //0x01 = Set Packet Size Message.
	MessageAbort                                 = 2 //0x02 = Abort.
	MessageAcknowledgement                       = 3 //0x03 = Acknowledge.
	MessageUserControl                           = 4 //0x04 = Control Message.
	MessageAcknowledgementWindowSize             = 5 //0x05 = Server Bandwidth
	MessageSetPeerBandwidth                      = 6 //0x06 = Client Bandwidth.
	//0x07 = Virtual Control.
	MessageAudio                                 = 8 ///0x08 = Audio Packet.
	MessageVideo                                 = 9 //0x09 = Video Packet.
	MessageDataAMF3                              = 15 //0x0F = Data Extended. Data Message（数据消息，Message Type ID＝15或18） 当信息使用AMF0编码时，Message Type ID＝18，AMF3编码时Message Type ID＝15.
	MessageSharedObjectAMF3                      = 16 //0x10 = Container Extended. Shared Object Message(共享消息，Message Type ID＝16或19)：表示一个Flash类型的对象，由键值对的集合组成，用于多客户端，多实例时使用。当信息使用AMF0编码时，Message Type ID＝19，AMF3编码时Message Type ID＝16.
	MessageCommandAMF3                           = 17 //0x11 = Command Extended (An AMF3 type command). Command Message(命令消息，Message Type ID＝17或20)：
	MessageDataAMF0                              = 18 //0x12 = Data (Invoke (onMetaData info is sent as such)). Data Message（数据消息，Message Type ID＝15或18）
	MessageSharedObjectAMF0                      = 19 //0x13 = Container.
	MessageCommandAMF0                           = 20 //0x14 = Command (An AMF0 type command). Command Message(命令消息，Message Type ID＝17或20)：
	//0x15 = UDP
	MessageAggregate                             = 22 //0x16 = Aggregate 多个RTMP子消息的集合
)
...
func (c *conn) serve() error {
	...
	for {
		fmt.Println("-----------------------------readChunk--begin")
		if err := c.readChunk(); err == io.EOF {
			fmt.Println("-----------------------------readChunk--c.netconn.Close()")
			c.netconn.Close()
			return nil
		} else if err != nil {
			return err
...
func (c *conn) readChunk() error {
	fmt.Println("================readChunkHeader begin")
	header, err := readChunkHeader(c.bufr)////+++++timestamp+message length+ message type id +message stream id+++++

	fmt.Printf("==========after======readChunkHeader , header.MessageHeader.MessageLength %d  \n" , header.MessageHeader.MessageLength)
	if err == io.EOF {
		c.server.logf("Got EOF")
		fmt.Println("-----------------------------readChunk--EOF")
		return nil
	} else if err != nil {
    ...
    c.server.logf("     read chunk header : header.MessageHeader.MessageTypeID : %d ",header.MessageHeader.MessageTypeID)
	...
	case MessageDataAMF0:
		payload := make([]byte, header.MessageHeader.MessageLength)
		msg_len := int(header.MessageHeader.MessageLength)
		read_size :=0
		...
		n, err := io.ReadAtLeast(c.bufr, payload,read_size)
		if err != nil {
			//// If it did, log them
		    log.Fatalln(err)
			return err
		}
		c.server.logf("Catch DataMessage(AMF0) metadata ,current read size %d ",n)
        if n < read_size {
			c.server.logf("err Catch DataMessage(AMF0) metadata ,should read  %d ", read_size)
		}
